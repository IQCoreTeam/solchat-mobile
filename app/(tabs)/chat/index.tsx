...
export default function TabSettingsScreen() {
  ...
  const handleJoinChatServerWrapper = async (
    ...
    try {
      ...
      return {
        message: "Failed to join chat server. Please try again.",
        subscriptionId: null,
      };
    }
  };
  const handleCreateChatServerWrapper = async (
    connection: Connection,
    serverId: string,
    pubkey: string | null,
    signAndSendTransaction: (
    ...
    try {
      ...
      const result = await handleCreateChatServer(
        connection,
        serverId,
        pubkey,
        signAndSendTransaction
  ...
  const onNewMessage = (msg: string, isHistorical = false) => {
    ...
    const processedOutput = isSystemMessage
      ? processedMsg
      : `[Chat] ${processedMsg}`;
    const now = Date.now();
    setHistory((prev) => {
